...
impl VanillaSwap
{
    ...
    //                         );
    // }
    pub fn get_models_variables_values(&self,date:f64,live_models:&HashMap<String,LiveModel>) -> Vec<f64>
    {
        ...
        for s in 0..num_scenarios
        {
            ...
            for (_k,v) in models.iter()
            {
                let values:Vec<f64>=v.get_variable_values(s, date);
    ...
    fn get_direct_value_for_leg(&self,leg_no:usize,scenario:usize,date:f64,models:&Vec<(&model::live_model::LiveModel,&model::live_model::LiveModel,&model::live_model::LiveModel)>) -> f64
    {
        let mut v:f64=0.0;
        for t in 0..self.legs[leg_no].payment_dates.len()
        {
            if self.legs[leg_no].payment_dates[t]>date
            {
                ...
                else
                {
                    ...
                    if cpn_start>date
                    {
                        fwd=model::live_model::get_fwd_rate(&models[leg_no].1, scenario, date, cpn_start, cpn_end);
                    }
                    else
                    {
                        fwd=models[leg_no].1.get_value(scenario,cpn_start,cpn_end-cpn_start).unwrap()
                    }                   
                    fwd*cpn_period+self.legs[leg_no].fixed_values[t]
                    //fwd+self.legs[leg_no].fixed_values[t]
                };
                leg_payment*=self.legs[leg_no].notional;
                let leg_df=(-cpn_time*models[leg_no].0.get_value(scenario, date, cpn_time).unwrap()).exp();
        ...
        let leg_fx=models[leg_no].2.get_value(scenario, date,0.0).unwrap();

        let v=v*leg_fx;
        return v;
    }

    fn get_direct_value(&self,scenario:usize,date:f64,models:&Vec<(&model::live_model::LiveModel,&model::live_model::LiveModel,&model::live_model::LiveModel)>) -> f64
    {
        let mut v:f64=0.0;
        for l in 0..self.legs.len()
        {
            let leg_value=self.get_direct_value_for_leg(l,scenario,date,models);
            v+=leg_value;
    ...
    pub fn get_value(&self,scenario:usize,date:f64,live_models:&HashMap<String,LiveModel>) -> f64
    {
        let models=self.get_live_models(&live_models);
        return self.get_direct_value(scenario,date,&models);
    }

    fn get_cashflows_for_leg(&self,leg_no:usize,scenario:usize,min_date:f64,max_date:f64,live_models:&HashMap<String,LiveModel>) -> Vec<(f64,f64)>
    {
        ...
        for t in 0..self.legs[leg_no].payment_dates.len()
        {
            if self.legs[leg_no].payment_dates[t]>min_date&&self.legs[leg_no].payment_dates[t]<=max_date
            {
                ...
                {
                    models[leg_no].1.get_value(scenario, cpn_start, cpn_period).unwrap()*cpn_period+self.legs[leg_no].fixed_values[t]
                };
                leg_payment*=self.legs[leg_no].notional;
                if self.legs[leg_no].pay_or_receive=="pay" { leg_payment=-leg_payment; } 
                let leg_fx=models[leg_no].2.get_value(scenario,self.legs[leg_no].payment_dates[t],0.0).unwrap();
    ...
    pub fn get_cashflows(&self,scenario:usize,min_date:f64,max_date:f64,live_models:&HashMap<String,LiveModel>) -> Vec<(f64,f64)>
    {
        let mut cashflows:Vec<(f64,f64)>=Vec::new();
        for l in 0..self.legs.len()
        {
            let mut cf=self.get_cashflows_for_leg(l, scenario, min_date, max_date, live_models);
...
impl Instrument for VanillaSwap
{
    ...
    fn compute_values(&self,start:usize,result_cube:&mut Cube,live_models:&HashMap<String,LiveModel>,logger:&Logger) -> (Vec<Vec<(f64,f64)>>,Cube)
    {
        ...
        {
            let mut scenario_cashflows=self.get_cashflows(s, 0.0, 9999.0, &live_models);
            ...
            for dt_idx in 0..result_cube.dates.len()
            {
                ...
                let v=self.get_direct_value(s,date,&models);
